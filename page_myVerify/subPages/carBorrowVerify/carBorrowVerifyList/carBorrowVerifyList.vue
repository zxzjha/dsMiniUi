<template>
	<view class="page-car-borrow-verify-list">
...
<script>
	...
	export default {
		name:'car-borrow-verify-list', // 借车审核订单列表
		data() {
			return {
				ongoing:'',
				tableData:[],
		...
		methods:{
			...
			async getDataList(){ // 获取我的借车审核订单
				...
				const res = await apiLib.getVerifyOrderList(params)
			...
			toOrderDetailPage(item){ // 去查看审核订单详情
				const refreshData = this.refreshData
				uni.navigateTo({
					url:'/page_myVerify/subPages/carBorrowVerify/carBorrowVerifyDetail/carBorrowVerifyDetail',
					events:{
						getFormData(data){ // 
							refreshData()
						}
